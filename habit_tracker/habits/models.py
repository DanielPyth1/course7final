...
class Habit(models.Model):
    ...
    action = models.CharField(max_length=255, verbose_name="Действие", null=True, blank=True)
    is_pleasant = models.BooleanField(default=False, verbose_name="Приятная привычка")
    related_habit = models.ForeignKey(
        'self', null=True, blank=True, on_delete=models.SET_NULL, verbose_name="Связанная привычка",
        related_name="related_to"
    )
    periodicity = models.PositiveIntegerField(default=1, verbose_name="Периодичность выполнения (в днях)")
    ...
    def clean(self):
        ...
        if self.periodicity > 7:
            raise ValidationError("Периодичность выполнения должна быть не реже 1 раза в 7 дней.")
